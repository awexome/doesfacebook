...
DoesFacebook is built against Rails versions 3.1 and higher. It depends on +gem "rails", ">= 3.1.0"+
for usage. Development requirements include bundler, jeweler, and rspec.
...
Facebook applications are created at defined within the 
...
application will run (development, staging, production, etc.), but you are not tried to this
...
* +profile_pic(fb_id, opts={})+ - Builds out an image tag containing the profile picture of the specified user. Pass +type+ and basic +image_tag+ options along in the +opts+ hash to customize.
* +fb_init(opts={}, &block)+ - Bootstrap an FB JS SDK implementation with this call, which will automatically inject the JS SDK with proper values for +appId+ as well as the proper browser-compatibility +channelUrl+ file.

See how this sample +fb_init+ invocation:

  <%= fb_init() %>

Generates this properly-formed JS SDK boot in your layout:
