  ...
  module Config
    ...
    def facebook_config
      ...
      if app_settings
        ...
        if app_config
          app_config["namespace"] ||= app_config["canvas_name"]
          Rails.logger.info("  Facebook configuration for app \"#{app_name}\" loaded via request to host #{request.host}")
          Rails.logger.warn("  DoesFacebook Deprecation Warning: \"canvas_name\" field in doesfacebook.yml should be renamed \"namespace\" as per Facebook Roadmap") unless app_config["canvas_name"].nil?
          return @@facebook_config = HashWithIndifferentAccess.new(app_config)
        end
      end

      app_config = all_facebook_config[Rails.env]
      unless app_config.blank?
        app_config["namespace"] ||= app_config["canvas_name"]
        Rails.logger.warn("  DoesFacebook Deprecation Warning: \"canvas_name\" field in doesfacebook.yml should be renamed \"namespace\" as per Facebook Roadmap") unless app_config["canvas_name"].nil?
        Rails.logger.info("  Facebook configuration loaded for app based on environment \"#{Rails.env}\"")
        @@facebook_config = HashWithIndifferentAccess.new(app_config)
      else
